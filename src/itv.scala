...
//def itv_pow( b : itv_t,  c : itv_t) : itv_t = itv_pow(b,c)



def itv_init : itv_t = new itv_t(bound.bound_init, bound.bound_init)
...
def itv_init_set(b : itv_t) : itv_t = new itv_t(b.inf, b.sup)

def itv_set(b : itv_t) : itv_t  = itv_init_set(b)
/*
def itv_set_num(itv_t a, num_t b)
{
...
def itv_set_bottom : itv_t =  new itv_t(bound_set_int(-1), bound_set_int(-1))

def itv_set_top : itv_t = new itv_t(bound_set_infty(1), bound_set_infty(1))
...
//FIXME: HERE
/*def itv_is_bottom(itv_internal_t* intern, itv_t a)
{
...
def itv_is_zero(a : itv_t) : Boolean = bound_sgn(a.inf)==0 && bound_sgn(a.sup)==0

def itv_is_leq(a : itv_t, b : itv_t) : Boolean = bound_cmp(a.sup,b.sup)<=0 && bound_cmp(a.inf,b.inf)<=0

def itv_is_eq(a : itv_t, b : itv_t) : Boolean = bound_equal(a.sup,b.sup) && bound_equal(a.inf,b.inf)


def itv_meet(/*itv_internal_t* intern, */b : itv_t, c : itv_t) : (itv_t, Boolean) = {
  val a = new itv_t(bound_min(b.sup,c.sup), bound_min(b.inf,c.inf))
  //FIXME: HERE2
  //return itv_canonicalize(intern,a,false)
  return (a, false)
  }

def itv_join(b : itv_t, c : itv_t) : itv_t = new itv_t(bound_max(b.sup,c.sup), bound_max(b.inf,c.inf))
...
def itv_widening(b : itv_t, c : itv_t) : itv_t = 
  new itv_t(sup = bound_widening(b.sup,c.sup), inf = bound_widening(b.inf,c.inf))

def itv_add(b : itv_t, c : itv_t) : itv_t =  
{ new itv_t(sup = bound_add(b.sup,c.sup), inf = bound_add(b.inf,c.inf))
}
